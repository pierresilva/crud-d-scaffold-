...
class CrudScaffold
{
    ...
    private function setupACL()
    {
        foreach ($this->data->models as $model) {
            ...
            $moduleNameForCode = NameResolver::solveName($this->module, 'name_name');
            $modelNameForCode = NameResolver::solveName($model->name, 'name_names');

            $permissionGroup = UserPermissionGroup::where('code', $moduleNameForCode)->first();
            if (!$permissionGroup) {
                $permissionGroup = UserPermissionGroup::create([
                    'name' => Str::upper($this->module),
            ...
            foreach ($permissions as $permission) {
                ...
                $permissionCode = $modelNameForCode . '.' . $permission;
